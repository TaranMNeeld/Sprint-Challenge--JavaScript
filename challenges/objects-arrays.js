...
console.log("The tyrannosaurus lived in the time period: " + tyrannosaurus.period);
...
for (let i = 0; i < graduates.length; i++) {
  let grad = graduates[i];
  let name = grad.first_name;
  let studentEmail = grad.email;
  contactInfo.push(`${name} ${studentEmail}`);
}
...
for (let i = 0; i < graduates.length; i++) {
  let grad = graduates[i];
  let univers = grad.university;
  let uniString = univers.toLowerCase();
...
lowerCase = zooAnimals.map(animal => {
    return "LowerCase Name: " + animal.animal_name.toLowerCase();
});
